...
Phase 3 verification passes 67 tests / 316 assertions, Blade compilation, Pint, Composer validation, npm install/audit, production Vite build, route inspection, application inspection, and real HTTP login/dashboard/assets/logout checks. Browser automation failed to start twice because its Windows sandbox helper exited, so pixel-level desktop/mobile visual inspection remains pending. Static responsive rules, Bootstrap offcanvas markup, and behavior-focused tests were verified. Phase 3 was approved before News CMS implementation began.



## Phase 4 — News CMS conventions

News category management uses authenticated resource routes under `admin.news-categories.*`; News uses `admin.news.*`. Public News is available at `/berita` and `/berita/{slug}` through `news.index` and `news.show`. Public detail lookup is explicitly constrained by the News `published` scope and returns 404 for draft, future, undated, or soft-deleted records.

`App\Support\UniqueSlug` is the small shared slug convention. Category slugs regenerate when the category name changes. News slugs regenerate when the title changes. Collisions receive numeric suffixes beginning with `-2`; News checks include soft-deleted rows so restoration cannot collide. Historical URL redirects are outside the current scope.

News publication uses only `draft` and `published`. Publishing without a supplied date sets `published_at` to the current time. An explicitly supplied date is retained. Returning a News record to draft preserves its previous publication timestamp. The creator is assigned from the authenticated administrator and remains unchanged when another administrator edits the record.

Thumbnails use Laravel's `public` disk under `news/thumbnails`. Validation accepts JPEG, PNG, and WebP images up to 4 MB. A new file is stored before the database transaction; it is removed if persistence fails. After a successful replacement, the former file is removed. Soft deletion retains the file because future restoration remains possible. The public storage link is required.

News content remains plain text. Admin forms use a textarea, and public output escapes the content before converting line breaks. Raw administrator HTML is not rendered. Public detail access increments the integer `views` counter once per successful request. Related content is limited to other currently published News in the same category.

Admin News listing eagerly loads category data and supports combined `search`, `status`, and `category` query parameters, newest-created ordering, 15-record server pagination, and preserved query strings. Category listing uses `withCount('news')`, search by name, sort order followed by name, and 15-record pagination. A category referenced by active or soft-deleted News cannot be deleted and receives a friendly Indonesian error.

Phase 4 follows the canonical CRUD pattern for subsequent modules: resource routes inside the authenticated group, Form Requests, focused controllers, shared Phase 3 Blade components, reusable create/edit form partials, Eloquent eager loading, isolated filesystem tests with `Storage::fake('public')`, and no repository or service layer for straightforward CRUD.

## Combined Phases 5–8: publishing and file lifecycle

Announcement, Agenda, Document Category, Document, and Gallery Album admin routes are authenticated resources without show actions. Their route prefixes are admin.announcements.*, admin.agendas.*, admin.document-categories.*, admin.documents.*, and admin.gallery.*. Gallery photo operations use admin.gallery.photos.store/update/destroy and verify that the photo belongs to the supplied album. All authenticated administrators share access; original authors are preserved on update. Only validated editable attributes enter persistence. Laravel's TrimStrings middleware trims normal text input.

Public pages use announcements.*, agendas.*, documents.*, and gallery.*:
- /pengumuman, /pengumuman/{slug}, /pengumuman/{slug}/lampiran
- /agenda, /agenda/{slug}
- /berkas, /berkas/{document}/download
- /galeri, /galeri/{slug}

Announcement visibility requires published status, a non-null publication date at or before now, no soft deletion, and either no expiration or expires_at strictly after now. Expired detail and attachment requests return 404. Expiration validation compares against the effective publication date, including an existing date or automatic now value. PublicationDate resolves explicit dates first, preserves prior dates when blank, and supplies now on first publication. The same date convention applies to Documents. Agenda and Gallery have no published_at column and use published status alone. Agenda also excludes soft-deleted rows.

Agenda times use Asia/Jakarta. Public listings separate upcoming/ongoing events from past events using COALESCE(end_at, start_at): events remain current until their end time when provided. Upcoming/ongoing events sort by start_at ascending; past events sort descending. Each section has an independent 12-item pagination parameter. End time cannot precede start time. No calendar library is required.

Announcement attachments and Document files use the existing local disk rooted at storage/app/private, with serve disabled. Direct public storage URLs are never generated for these files. Directories are announcements and documents. Accepted extensions are PDF, DOC, DOCX, XLS, XLSX; Laravel validates both extensions and detected MIME. Limits are 10 MB for announcements and 15 MB for documents. Random physical names are generated by Storage, and sanitized original filenames are used for download headers. Documents also store detected MIME and byte size. HumanFileSize provides presentation formatting without a package.

Controlled downloads always query public visibility and check physical existence before creating a download response. Invalid or missing files return 404. Document download_count increments atomically once per accepted request, after the response is prepared; this measures requests, not guaranteed completed transfers. Announcements have no download counter. Published Documents remain visible when their category becomes inactive. New Documents select active categories; edit forms also retain the current inactive category. Category deletion is refused while any Document references it, including soft-deleted rows.

FileUploads contains shared extension/MIME/size rules, safe storage with explicit failure handling, original filename normalization, and deduplicated cleanup. Controllers keep lifecycle coordination explicit: write new file, persist metadata in a transaction, clean new files on rollback, remove replaced files only after success. Announcement and Document soft deletion retains files. Physical deletion failures are reported to the application log; there is no background cleanup service.

Gallery images use the public disk under gallery/{album-id}. JPEG, PNG, and WebP are allowed, up to 5 MB each. Upload batches contain at most 10 photos, with an optional separate cover on album creation. All files validate before persistence. Album plus photo creation and batch addition use database transactions with rollback cleanup of all newly written files. Photo metadata forms have photo-specific input keys so validation does not populate unrelated photo or album fields.

Gallery photos sort by sort_order then id. The firstPhoto relation selects one ordered photo for cover fallback without loading an entire album. Index pages eager load that relation and use withCount('photos'). An explicit existing cover wins; otherwise the first photo is used, then an internal placeholder. Missing image files render a clean placeholder. Public photo grids and admin photo forms paginate at 24. Full image links retain the original files; no lightbox package or image processing is installed.

Gallery deletion is permanent. Album deletion collects cover/photo paths, deletes database rows in a transaction (with the existing photo foreign-key cascade), then removes files after successful commit. Individual photo deletion likewise removes the file after the database operation and clears a matching cover reference. An old cover shared with a retained photo is not removed during cover replacement.

UniqueSlug remains shared by News and the new modules. Its generated base is limited to 175 characters so suffixes fit the existing 190-character slug columns. Title/name changes regenerate slugs. Soft-deletable models include trashed records in collision checks. No schema changes or historical redirect system are needed.

All admin lists use GET search, relevant filters, 15-row server pagination, and preserved query strings. Public Announcements/Documents/Gallery paginate at 12. Existing Blade components, Indonesian dates, confirmation modal, flash messages, public layout, and Bootstrap styles remain the UI foundation. Plain text is escaped in Blade and uses CSS white-space for line breaks. No new packages, APIs, homepage sections, profile editing, banner editing, settings editing, or user management are part of this implementation.

## Village profile, officials, banners, and homepage conventions

The Settings editor uses GET/PATCH /admin/village-profile (admin.village-profile.edit/update). Its grouped form maps explicit field names to the existing 16 approved keys in SiteSettings::FIELDS. The application owns key/type/group metadata; arbitrary request keys never create configuration. Optional blank text becomes null, while omitted logo/SOTK uploads preserve stored paths. Updates across keys are transactional. Settings values remain plain text; mission preserves line breaks without converting prose into list items. The form does not edit head identity or application environment configuration.

SiteSettings batches approved values in one query and memoizes them in a request-owned object. A view composer supplies that object only to public views, layout, and partials; it does not query settings during unrelated admin or Artisan requests. There is no cross-request settings cache to invalidate, so edits appear on the next public request. Missing site identity falls back to APP_NAME. Missing logo files fall back to text branding. New public templates escape all database content.

SafeUrl validates HTTP/HTTPS links, rejecting credentials, control characters and unsafe protocols. Banner CTAs require a URL when a label is supplied; only complete valid label/URL pairs render publicly. Internal links can be entered as full HTTP/HTTPS URLs. Social URLs are checked on both input and output. Map URLs allow only HTTPS Google Maps embed paths on www.google.com/maps/embed or maps.google.com/maps/embed (including path descendants), or www.openstreetmap.org/export/embed.html. No iframe HTML is accepted. The footer renders a controlled lazy iframe with title, referrer policy and sandbox attributes, and omits invalid or missing maps. Phone is escaped text rather than a generated URI; valid email may use mailto. External links open in the same tab.

VillageOfficial is the only source for head name, position and photo. Authenticated CRUD lives at /admin/village-officials (admin.village-officials.*), with name/position search, active filter, 15-item pagination, and sort_order/id ordering. Assigning an active head clears all other head flags within a transaction. Setting an official inactive clears their own head flag. A short Laravel atomic cache lock serializes official mutations, including initially empty tables; row locks protect reloaded records. Keep a lock-capable cache driver. The current file cache coordinates this single-host application; any future multi-host setup must share a lock-capable cache. No permission system or schema uniqueness workaround is introduced. Biography is limited to 15,000 characters to fit the existing MySQL TEXT column even with four-byte characters.

Official deletion is permanent and may delete the current head. The shared confirmation warns about missing head identity until a replacement is assigned. Public views select active heads only and remain stable without one. Photos use the public disk under officials, maximum 5 MB, JPEG/PNG/WebP. Logo and SOTK use settings/logo and settings/sotk on the same disk with the same image rules. New files are removed if persistence fails; replaced files are removed after database success. Hard deletion likewise removes physical files only after successful database deletion.

Banner CRUD lives at /admin/banners (admin.banners.*), with title/subtitle search, active filter, sort_order/id ordering and 15-item pagination. Image is required on create and optional on update, stored on the public disk under banners, using the existing 5 MB image rules. Banner active() still checks the boolean only. currentlyVisible() adds starts_at null or <= now and ends_at null or > now. Equal start/end is rejected when both are provided. Scheduling uses the existing Asia/Jakarta timezone. Hard deletion and replacement use the same successful-persistence-first file lifecycle.

Public profile routes are /profil (profile.index), /profil/sotk (profile.organization), and /profil/perangkat-desa (profile.officials). Vision, mission and history share the main profile page with section anchors. SOTK uses contain sizing and offers the original image; absent files show an empty state. The officials grid shows active rows only, ordered by sort_order/id, paginated at 12, with a subtle head badge, biography excerpt and local photo fallback.

HomeController coordinates bounded read queries without a service layer. Display order is village identity/hero, optional head welcome, latest news, announcements, upcoming/ongoing agenda, documents, gallery, then shared contact/footer. Limits: 10 currently visible banners ordered sort_order/id; 3 news; 4 announcements; 4 agendas; 4 documents; 3 gallery albums. News/announcements/documents reuse publication scopes and order by published_at descending then id. Agenda uses COALESCE(end_at,start_at) >= now and ascending start_at/id. Gallery uses its publication scope, event_date/id descending, firstPhoto eager loading and photos count. Category relations are eager loaded for News/Documents. All sections handle empty results, and missing physical images use local fallback presentation. Document links always use the controlled download endpoint and homepage reads do not increment counters.

Zero banners render a neutral welcome hero. One banner has no carousel controls. Multiple banners use the already installed Bootstrap carousel with labelled previous/next controls and no automatic rotation. Reduced-motion CSS disables slide transitions. On smaller screens, banner copy follows the image to avoid obscuring content; larger screens use a restrained readable overlay. The profile dropdown, dynamic logo, shared footer and existing module links are active. Settings, Officials and Banner admin sidebar entries are active; Users remains disabled. The Dashboard official summary links to the new list without altering its counts.

No new schema, dependencies, frameworks, user-management routes, or excluded modules were added. Validation and lifecycle behavior use SQLite in-memory and fake storage in tests; development MySQL remains untouched by automated tests. Further visual refinement, production configuration and deployment belong to the separately authorized final phase.
## Shared UI refinement conventions

Public pages use a responsive 2.75–4.5 rem section spacing token, restrained three-quarter-rem card radius, bounded headings, and a 52 rem reading width for long prose. Shared primary/outline controls, checkboxes, dropdown selection and pagination use the central green palette. Long titles, addresses, filenames, breadcrumbs and buttons wrap rather than forcing viewport overflow. Empty listings use the public-empty-state surface. News thumbnails and album covers remain 16:9; gallery photos use the shared media-landscape 4:3 class; official portraits remain 3:4; logo/SOTK and admin upload previews use contain sizing. News views now check physical thumbnail existence before generating image URLs, matching the other public modules. Plain-text article formatting uses escaped Blade output and the shared public-plain-text class.

Hero copy remains below the image on mobile. Desktop slides use overlapping grid cells instead of absolutely positioned copy, allowing unusually long text to contribute to slide height. Carousel controls remain manual and reduced motion is respected. The public skip link, active navigation underline and explicit focus outlines aid keyboard navigation. Admin table wrappers are labelled focusable regions with horizontal scrolling, captions and column header scopes; table content stays readable instead of shrinking into narrow columns. Destructive buttons include record names in their accessible labels, and the confirmation dialog describes the selected record. Shared form labels/errors remain associated with their controls. Empty-state titles use styled paragraphs to avoid accidental skipped heading levels. Keep UI refinements in existing styles/components; routes, queries, validation, storage and publication rules remain unchanged.
## Phase 14 prototype-informed visual direction

The supplied reference lives in resources/views/desaku: ten HTML pages, style.css, and eight local image assets. Reusable patterns are the green village identity, compact navigation, wide hero, portrait-and-welcome composition, short section underlines, image-led news cards, profile hierarchy, organization chart, gallery grid and structured footer. Berita2 is an alternative combined layout, not a new module. Potensi, Pengaduan, UMKM and Produk remain excluded. Prototype files are untouched and are not linked or included by production views.

Production uses CMS identity and uploads, not the prototype's Kaliprau logo, generic avatar, large background photographs, fake statistics or hardcoded articles. No external fonts, icon CDN, duplicate Bootstrap, automatic carousel, fixed-height scrolling welcome or decorative motion was imported. System typography, centralized green/neutral tokens and subtle borders keep the official information hierarchy readable.

resources/css/app.css owns Bootstrap and shared tokens/controls/accessibility. It imports resources/css/public.css for public presentation; admin.css retains independent admin styling with only light table, form, sidebar and dashboard refinement. Shared public Blade components own page headers, section headings and News/Announcement/Agenda/Document/Gallery previews, reused between homepage and listings. Reading widths, image ratios, mobile stacking, empty states and optional footer content remain consistent. Welcome portraits use 4:5; official cards retain 3:4; news and album covers use 16:9; gallery photos use 4:3. SOTK remains uncropped with a full-image link.

Phase 14 changes presentation only: no routes, business queries, publication rules, controllers, models, schema, dependencies or JavaScript changed. Regression verification passed 153 tests (1696 assertions), Vite build and Blade compilation. Browser automation was attempted but failed during Windows sandbox kernel setup; responsive layouts and focus treatment were reviewed in source, not certified by a rendered browser or screen reader. Complete desktop/tablet/mobile visual acceptance when browser tooling is available before production approval.

## Phase 14B full public redesign

Phase 14B replaces the conservative public presentation with a design derived directly from the supplied Beranda, Profil and Berita references. The production site now uses a 1240px desktop content frame, an image-led 30–38rem hero, deep village green surfaces, restrained orange accents, stronger editorial type, alternating section backgrounds, a dominant head welcome composition, featured News, differentiated notice/agenda/document components, mosaic-style Gallery grids and a dense multi-column footer. CMS images remain primary; designed local CSS fallbacks cover missing banners, thumbnails and portraits without remote assets or prototype identity.

Public styling remains in resources/css/public.css and is organized by tokens, navigation, hero, section rhythm, content types, profile, gallery, footer and responsive rules. The shared Blade components continue to serve homepage and listing views. The News index selects the first row already present in its paginator as the visual feature and removes it from that page's grid; no controller query changes. Prototype HTML, CSS, logo and photographs remain untouched in resources/views/desaku and are not served by production templates.

Rendered review used local Microsoft Edge screenshots for Homepage desktop/mobile, Profile desktop, News desktop and Gallery desktop after the UI automation kernel failed twice. Review found and corrected a narrow Gallery empty state, mobile menu icon visibility and oversized footer/component headings. Final source and rendered review cover breakpoints at 390, 768/992 behavior through CSS, and 1440 desktop; a real-device interaction pass remains appropriate before production release. Phase 14B adds no route, controller, model, migration, dependency, admin workflow or JavaScript changes.
